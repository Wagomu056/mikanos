...
alignas(kPageSize4K)
    std::array<std::array<uint64_t, 512>, kPageDirectoryCount> page_direcotory;
} // namespace

void SetupIdentityPageTable() {
  pml4_table[0] = reinterpret_cast<uint64_t>(&pdp_table[0]) | 0x003;
  for (int i_pdpt = 0; i_pdpt < page_direcotory.size(); ++i_pdpt) {
    pdp_table[i_pdpt] =
        reinterpret_cast<uint64_t>(&page_direcotory[i_pdpt]) | 0x003;

    for (int i_pd = 0; i_pd < 512; ++i_pd) {
      page_direcotory[i_pdpt][i_pd] =
